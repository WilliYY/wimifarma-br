...
import { campaignUrl, marketingIssues, merchantIssues, type MarketingProduct } from "@/features/products/marketing";
...
export function ProductMarketingTools({ product }: { product: MarketingProduct }) {
  ...
  return <><Button onClick={() => setOpen(true)} size="sm" type="button" variant="secondary"><Megaphone className="h-4 w-4" />SEO e marketing</Button>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Seu produto no Google</DialogTitle><DialogDescription>Ao publicar, esta página entra automaticamente no catálogo e no sitemap. Compartilhar links é opcional e não é necessário para o Google encontrar o produto.</DialogDescription></DialogHeader>
      ...
      <details className="rounded-lg border border-line p-3"><summary className="cursor-pointer py-2 text-sm font-bold">Vitrines do Google: {merchantIssues(product).length ? "conferir pendências" : "dados aptos para o catálogo"}</summary><p className="mt-2 text-xs leading-5 text-muted">Essa verificação é para Shopping. Sua página publicada continua disponível para a busca orgânica. A análise final é do Google.</p>{merchantIssues(product).length > 0 && <ul className="mt-3 list-disc space-y-2 pl-5 text-xs leading-5 text-amber-800">{merchantIssues(product).map(issue => <li key={issue}>{issue}</li>)}</ul>}</details>
