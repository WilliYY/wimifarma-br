...
export function ProductMarketingTools({ product }: { product: MarketingProduct }) {
  ...
  return <><Button onClick={() => setOpen(true)} size="sm" type="button" variant="secondary"><Megaphone className="h-4 w-4" />SEO e marketing</Button>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>SEO e marketing do produto</DialogTitle><DialogDescription>Confira a apresentação, prepare um link de campanha e copie o material para revisar.</DialogDescription></DialogHeader>
