...
export function merchantIssues(product: MarketingProduct) {
  ...
  const type = productTypeHint(`${product.category ?? ""} ${product.name}`);
  if (product.requiresPrescription || product.isPopularPharmacy || product.activeIngredients.length || !["beauty", "hygiene", "food"].includes(type)) issues.push("Tipo regulado ou não classificado: revisar separadamente as políticas do Google.");
...
export function buildMerchantFeed(products: MarketingProduct[]) {
  ...
  return { count: eligible.length, excluded: products.length - eligible.length, xml: `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0" xmlns:g="http://base.google.com/ns/1.0"><channel><title>Wimifarma - catálogo para revisão</title><link>${SITE_URL}</link><description>Produtos para revisão no Google Merchant Center.</description>${items.join("")}</channel></rss>` };
}
